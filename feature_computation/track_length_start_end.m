function track_length_start_end(mainfolder,subfolder)
% 
...
while (trackind <= numboftracks)
    ...
    track_length_start_end_matrix(trackind,1)=tracklength;
    track_length_start_end_matrix(trackind,2)=startind;
    track_length_start_end_matrix(trackind,3)=endind;
    track_length_start_end_matrix(trackind,4)=firstimage;
    track_length_start_end_matrix(trackind,5)=lastimage;
...
if (plotfigure == 2)
    figure(2)
    clf
    hist(track_length_start_end_matrix(:,1),1000)
    xlim([1,50])
end % if (plotfigure == 2)
...
featurename=['track_length_start_end'];

% Save the feature:
featurefolderfilename=[trackfolder,featurename,'.mat'];
save(featurefolderfilename,'track_length_start_end_matrix');
