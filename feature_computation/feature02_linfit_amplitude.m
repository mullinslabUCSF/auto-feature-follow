function feature01_linfit_slope(mainfolder,subfolder,mintrackleng,maxtrackleng,background)
% 
% Extract track feature and save the results to a file;
% Fit a linear function to the tracks and extract the slope;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         (tracklength < maxtrackleng || maxtrackleng == 0))
    ...
    % Normalize amplitude vector and time vector:
    %amplitudevecnorm=amplitudevec./nanmean(amplitudevec);
    amplitudevecnorm=amplitudevec;
    timevecauxnorm=timevecaux./timevecaux(end);
    
    
    % Interpolate the amplitude data around NaNs before fitting a function:
    nanixaux=find(isnan(amplitudevecnorm)==1);
    nonnanixaux=find(isnan(amplitudevecnorm)==0);
    amplitudevecnorminterp=amplitudevecnorm;
    if (isempty(nanixaux) == 0)
        amplitudevecnorminterp=interp1(nonnanixaux,amplitudevecnorm(nonnanixaux),timevecaux); 
    end % if (isempty(nanixaux) == 0)
    amplitudevecnorm;
    amplitudevecnorminterp;
    
    % Fit function to the amplitude:
    %options=optimset('Display','iter');   % Option to display output
    options=optimset('Display','off');   % Option to display NO output
    fitpar0=[-0.5,amplitudevecnorminterp(1)];
    [fitparout,resnorm,residual,exitflag]=...
        lsqcurvefit(@lin_fit_fun,fitpar0,timevecauxnorm,log(amplitudevecnorminterp),[],[],options);
    exitflag;

    feature=exp(fitparout(2))-background;
    
    if (plotfigure == 1)
        fitplot=lin_fit_fun(fitparout,timevecauxnorm);    
        clf
        hold on
        plot(timevecauxnorm,amplitudevecnorm,'x-')
...
% function ydata=exp_fit_fun(par,xdata)
%     ydata=exp(par(2)).*exp(par(1).*xdata);
% end % function
