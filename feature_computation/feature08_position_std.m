function feature08_position_std(mainfolder,subfolder,mintrackleng,maxtrackleng)
% 
...
%         (tracklength < maxtrackleng || maxtrackleng == 0))
    ...
    xposvec=tracksFinal(trackind).tracksCoordAmpCG(1:8:end);
    yposvec=tracksFinal(trackind).tracksCoordAmpCG(2:8:end);
    xstd=nanstd(xposvec);
    ystd=nanstd(yposvec);
    
    feature=sqrt(xstd^2+ystd^2);
